    ...
    NSInteger index = -1;
       for (BMChineseSortModel* object in sortModelArray) {
          NSString *firstLetter = [object.pinYin substringToIndex:1];
           if ([firstLetter containsString:@"#"]) {
               index = [sortModelArray indexOfObject:object];
               NSLog(@"-----------%ld",(long)index);
           }
        }
        if (index > -1) {
            BMChineseSortModel * model = sortModelArray[index];
            [sortModelArray removeObject:model];
            [sortModelArray addObject:model];
        }
